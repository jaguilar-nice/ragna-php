...
class Mail {
	...
	function __construct($from = null)
	{
		...
		if ($from == null)
		{
			$this->mail_from = Rag::$info->smtp->from;
		}
		else
		{
			$this->mail_from = $from; 
		...
		$this->set_smtp_host(Rag::$info->smtp->host, Rag::$info->smtp->port);
		$this->set_smtp_auth(Rag::$info->smtp->user, Rag::$info->smtp->pass);
	}
	 ...
	 * @return void
	 */
	function set_html($html, $cabeceras = false)
	{
		if (!empty($html)){
			if ($cabeceras)
			{
				$this->mail_html = file_get_contents("../generico/mail/cabecera.php") . nl2br($html) . file_get_contents("../generico/mail/cierre.php");
			}
			else
			{
				$this->mail_html = nl2br($html);
			}
		}
	}
